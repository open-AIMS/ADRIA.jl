...
function ADRIA.viz.dhw_scenario(
    ...
    axis_opts[:xticks] = get(axis_opts, :xticks, unique(vcat(years[1:5:end], years[end])))
    ax = Axis(f[1, 1]; axis_opts...)

    each_loc = series!(ax, years, Matrix(loc_scens'); solid_color=(:red, 0.05))
    scen_mean = lines!(ax, years, collect(mean_dhw_scen); color=(:black, 0.8))
...
function ADRIA.viz.dhw_scenarios(
    ...
    each_scen = series!(
        ax, years, dropdims(mean(data; dims=2); dims=2).data';
        solid_color=(:red, 0.2)
    )
