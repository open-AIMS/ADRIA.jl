...
"""
    scenario!(f::GridPosition, rs::ADRIA.ResultSet, y::AbstractArray; opts=Dict(by_RCP => false), axis_opts=Dict(), series_opts=Dict())
...
GridPosition
"""
function scenario!(f::GridPosition, rs::ResultSet, y::AbstractArray;
    opts::Dict=Dict(:by_RCP => false), axis_opts::Dict=Dict(), series_opts::Dict=Dict())
...
"""
    scenario(rs::ADRIA.ResultSet, y; opts=Dict(by_RCP => false), fig_opts=Dict(), axis_opts=Dict(), series_opts=Dict())

# Arguments
- `rs` : ResultSet
- `y` : AbstractArray, of scenario outcomes
- `opts` : Aviz options  
    - `by_RCP`, color by RCP otherwise color by scenario type. Defaults to false.
- `fig_opts` : Additional options to pass to adjust Figure creation  
  See: https://docs.makie.org/v0.19/api/index.html#Figure
...
Figure
"""
function scenario(rs::ResultSet, y::AbstractArray; opts::Dict=Dict(:by_RCP => false), fig_opts::Dict=Dict(), axis_opts::Dict=Dict(), series_opts::Dict=Dict())
    f = Figure(; fig_opts...)
