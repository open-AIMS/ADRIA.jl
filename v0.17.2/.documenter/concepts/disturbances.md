...
## Bleaching mortality {#Bleaching-mortality}
...
## Functional group and size class differences {#Functional-group-and-size-class-differences}
...
- **Settler tolerance**: Newly seeded corals can carry a user-specified thermal tolerance offset (`a_adapt`), representing a given level of enhanced thermal tolerance, through assisted gene flow, adaptation or other enhancement process.
  

The mean tolerance is hard-capped at `initial_mean + HEAT_UB` to represent a biological ceiling on adaptation.

## Fogging and Solar Radiation Management (SRM) {#Fogging-and-Solar-Radiation-Management-SRM}
...
This reduction is applied before bleaching mortality is computed, so treated locations experience reduced bleaching proportional to the intervention intensity.
