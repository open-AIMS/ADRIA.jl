...
# ADRIA Docker Setup {#ADRIA-Docker-Setup}
...
- [docker configuration](/development/docker#docker-configuration)
  
- [adria-base](/development/docker#adria-base)
  - [A note about platform build targets](/development/docker#a-note-about-platform-build-targets)
    
  - [Published base images](/development/docker#published-base-images)
    
  - [Building `adria-base`](/development/docker#building-adria-base)
    
  - [Running `adria-base` with a non-interactive Julia command](/development/docker#running-adria-base-with-a-non-interactive-julia-command)
    
  - [Running `adria-base` as an interactive Julia shell](/development/docker#running-adria-base-as-an-interactive-julia-shell)
    
  - [Running `adria-base` with a non-Julia entrypoint](/development/docker#running-adria-base-with-a-non-julia-entrypoint)
    
  - [Deriving an image from `adria-base`](/development/docker#deriving-an-image-from-adria-base)
    
  
- [adria-dev](/development/docker#adria-dev)
  - [Building `adria-dev`](/development/docker#building-adria-dev)
    
  - [Running `adria-dev` as an interactive Julia shell](/development/docker#running-adria-dev-as-an-interactive-julia-shell)
    
  - [Running ADRIA tests with `adria-dev`](/development/docker#running-adria-tests-with-adria-dev)
    
  
- [adria-sandbox](/development/docker#adria-sandbox)
  - [Building `adria-sandbox`](/development/docker#building-adria-sandbox)
    
  - [Running `adria-sandbox`](/development/docker#running-adria-sandbox)
    
  - [Interacting with `adria-sandbox`](/development/docker#interacting-with-adria-sandbox)
...
- `ADRIA_REPO`: URL for the repository that ADRIA.jl should be cloned from. Defaults to [https://github.com/open-AIMS/ADRIA.jl.git](https://github.com/open-AIMS/ADRIA.jl.git)
...
The `adria-dev` image variant is an _alternative_ to the `adria-base` image, not a derived application. Instead of installing [ADRIA.jl](https://github.com/open-AIMS/ADRIA.jl) as a normal package, it looks for the ADRIA.jl source code in a local subdirectory, and installs that as a [Julia development package](https://pkgdocs.julialang.org/v1/api/#Pkg.develop).

This allows you to use the `adria-dev` container as an `ADRIA.jl` development environment: you can run tests, bind-mount and edit the code, re-resolve dependencies and all sorts of other useful things without needing to a native installation of Julia.
...
Very useful for running commands to update Package manifests and similar! The command will activate the shared `@adria` environment by default, but you can switch to any other environment as you need to.
...
ADRIA tests can be run like any other Julia command, but the GOTCHA is that they need to run in the local project environment, NOT in the shared `@adria` environment:
...
This application uses the pre-installed ADRIA package, and can be built on _either_ of `adria-base` or `adria-dev`, depending on whether you want the development version of the package or not.

It depends on input and output data files which must be provided at runtime.

### Building `adria-sandbox` {#Building-adria-sandbox}

The sandbox build supports an optional `SANDBOX_FROM` build argument which is used to specify a base image. Supported values are `adria-dev` (the default) or `adria-base`.
...
The `adria-sandbox` image is configured to automatically run the `dev.jl` script when a container made from that image runs.
...
This documentation and the `docker-compose.yaml` file in the repository demonstrate using [bind mounts](https://docs.docker.com/storage/bind-mounts/) to data-directories which are local to your current working directory for this purpose, but [docker volumes](https://docs.docker.com/storage/volumes/) with any supported storage driver should also work fine.
...
The `dev.jl` script should run with any files you have provided in your input volume, and will create and output files in your output volume. The container will be removed once the script completes.
...
In this case, you may also like to bind-mount your sandbox and/or ADRIA.jl source code into the container, so that you can edit it without having to re-build the container each time.
