...
if !@isdefined(ADRIA_DOM)
    const ADRIA_DOM = ADRIA.load_domain(TEST_DOMAIN_PATH)
end

function get_example_domain()
    return ADRIA_DOM
end
...
function get_example_resultset()
    rs = try
        ...
        if !@isdefined(SMALL_TEST_RS)
            @eval const SMALL_TEST_RS = test_small_spec_rs()
        end
