...
@testset "Distance Sorting" begin
    ...
    rankings = zeros(Int64, length(location_order[:, 3]), 1)
    rankings .= location_order[:, 3]
    new_preflocations, new_rankings = distance_sorting(preflocations, location_order, dists, dist_thresh, rankings, 1)
    ...
    pref_dists = findall(dists[preflocations, preflocations] .< min_dist)
    rep_locations = sort(unique(reinterpret(Int64, pref_dists)))
    new_preflocations = distance_sorting(preflocations, location_order, dists, dist_thresh, rankings, 1)
