...
@testset "Distance Sorting" begin
    ...
    new_prefsites = distance_sorting(prefsites, site_order, dists, dist_thresh)

    n_diff_sites = length(setdiff(prefsites, new_prefsites))
    @test all([in(new_prefsites[k], site_order) for k in 1:5]) || "Not all sites are selected from the site list."
