...
function sample_pairwise(
    ...
    # Counterfactual twins: same environmental draws, intervention regime off.
    # Setting guided=-1 and re-resolving the guided dependency table row-wise
    # yields the same CF constants as sample_cf (strategy columns get the -1.0
    # sentinel, not 0.0), keeping paired CFs indistinguishable from sampled CFs.
    cf_scens = copy(interv_scens)
    cf_scens[!, :guided] .= -1.0
    _apply_guided_dependencies!(cf_scens)
