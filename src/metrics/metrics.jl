...
function _relative_taxa_cover(
    ...
    if n_group_and_size % n_groups != 0
        throw(
            ArgumentError(
                "Number of functional groups given does not divide n_group_and_size. n_group_and_size: $(n_group_and_size). n_groups: $(n_groups)"
            )
        )
    end
...
function _relative_loc_taxa_cover(
    ...
    if n_group_and_size % n_groups != 0
        throw(
            ArgumentError(
                "Number of groups must divide n_group_and_size. n_group_and_size $(n_group_and_size), n_groups: $(n_groups)"
            )
        )
    end
    n_sc::Int64 = Int64(n_group_and_size / n_groups)
