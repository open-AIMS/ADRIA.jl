...
Then orders locations from highest aggregate score to lowest.
...
Rank for a particular location is calculated as a ratio

    C = S_n/(S_p + S_n)

S_n = √{∑(criteria .- NIS)²}
    is the squareroot of the summed differences between the criteria for a location and the
    Negative Ideal Solution (NIS), or worst performing location in each criteria.
S_p  = √{∑(criteria .- NIS)²}
    is the squareroot of the summed differences between the criteria for a location and the
    Positive Ideal Solution (PIS), or best performing location in each criteria.
...
Rank for a particular location is calculated as a linear combination of ratios,
weighted by v:
    Q = v(Sr - S_h) / (S_s - S_h) + (1 - v)(R - R_h) / (R_s - R_h)

where
- Sr = ∑(PIS-criteria) for each location, summed over criteria.
- R = max(PIS-criteria) for each location, with the max over criteria.
- S_h = min(∑(PIS-criteria)) over locations, the minimum summed distance from
    the positive ideal solution.
- S_s = max(∑(PIS-criteria)) over locations, maximum summed distance from
    the positive ideal solution.
- R_h = min(max(PIS-criteria)) over locations, the minimum max distance from
    the positive ideal solution.
- R_s = max(max(PIS-criteria)) over locations, the maximum max distance from
    the positive ideal solution.
