...
"""
    sample_pairwise(dom::Domain, n::Int64; sample_method=SobolSample(R=OwenScramble(base=2, pad=32)))::DataFrame
    sample_pairwise(dom::Domain, n::Int64, sample_func::Function; kwargs...)::DataFrame

Generate paired scenarios.
For every scenario generated by `sample_func`, a corresponding counterfactual scenario 
is created with the same environmental factors but with all interventions disabled.

# Arguments
- `dom` : Domain.
- `n` : number of samples to generate (before doubling for counterfactuals).
- `sample_func` : Sampling function to use (e.g., `sample_guided`).
- `kwargs` : Arguments passed to `sample_func`.

# Returns
Scenario specification with 2*n rows, where rows 1:n are the counterfactual scenarios
and rows n+1:2n are their matching intervention scenarios.
"""
function sample_pairwise(
    dom::Domain, n::Int64, sample_func::Function; kwargs...
)::DataFrame
    # 1. Generate scenarios
    interv_scens = sample_func(dom, n; kwargs...)

    # 2. Create counterfactuals as a copy
    cf_scens = copy(interv_scens)

    # 3. Identify intervention and criteria factors to disable
    # These match the logic in deactivate_interventions!
    spec = model_spec(dom)
    iv_factors = component_params(spec, Intervention).fieldname
    criteria_factors =
        component_params(
            spec,
            [
    ...
    # 4. Disable all intervention-related factors
    # Set guided to -1 (Unguided/Counterfactual)
    cf_scens[!, :guided] .= -1.0

    # Set all other intervention factors to 0
    for f in iv_factors
        if f != :guided
            cf_scens[!, f] .= 0.0
        end
    ...
    # Set all criteria weights to 0
    for f in criteria_factors
        cf_scens[!, f] .= 0.0
    end

    # 5. Return combined scenarios
    # Return as [Counterfactuals; Interventions]
    # This matches the Linker's expectation of paired indices (i, i+n)
    return [cf_scens; interv_scens]
end
function sample_pairwise(
    dom::Domain, n::Int64; sample_method=SobolSample(; R=OwenScramble(; base=2, pad=32))
)::DataFrame
    return sample_pairwise(dom, n, sample_guided; sample_method=sample_method)
end
