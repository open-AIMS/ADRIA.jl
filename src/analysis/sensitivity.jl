...
function convergence(
    ...
    for (cc, factors) ∈ zip(components, target_factors)
...
function rsa(
    ...
    yax_store_default = Tuple(
        ...
        ) for _ ∈ 1:(length(factors) - length(unordered_cat))
    )
...
function outcome_map(
    ...
    X_q = zeros(S + 1)
    for (j, fact_t) ∈ enumerate(target_factors)
        X_f = X[:, fact_t]
