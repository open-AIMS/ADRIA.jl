...
function find_pareto_optimal(
    ...
    x_idx = [scens.RCP .== rcp for rcp ∈ rcps]
    r_rcp = [reduce(vcat, nds(y[rcp_idx, :], offset)) for rcp_idx ∈ x_idx]

    scen_ids = [findall(rcp_idx)[r_idx] for (rcp_idx, r_idx) ∈ zip(x_idx, r_rcp)]
