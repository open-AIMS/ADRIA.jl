...
function run_model(domain::Domain, param_set::NamedDimsArray, corals::DataFrame, cache::NamedTuple)::NamedTuple
    ...
    if .!all(site_data.depth_med .== 0)
        ...
        depth_criteria::BitArray{1} = depth_criteria_indx(site_data.depth_med, max_depth, param_set("depth_min"))
    ...
    for tstep::Int64 in 2:tf
        ...
        if is_guided && (in_seed_years || in_shade_years)
            ...
            mcda_vars.heat_stress_prob .= summary_stat_env(env_horizon, :scenario)

            @views env_horizon = decay[d_s] .* wave_scen[horizon, :]
            mcda_vars.dam_prob .= summmary_stat_env(env_horizon, :scenario)
        end
        if is_guided && (in_seed_years || in_shade_years)
