...
"""
    scale_factor_vec_to_array(sf_vec::AbstractVector{T}, n_taxa::Int64, n_spatial_groups::Int64, n_p_types::Int64)::Array{T, 3} where T <: Union{Float64, String, Tuple}

Reshape a scale factor vector to be an array of dimensions [taxa ⋅ param_type ⋅ spatial_groups].
Wrapper of reshape(vec, (n_taxa, n_p_types, n_spatial_groups)) for consistency. Allow for symbol
...
- `n_spatial_groups` : Number of spatial_groups sharing scaling factors.
- `n_p_types` : Number of parameters using scale coefficients

# Returns
- reshape(sf_vec, (n_taxa, n_p_types, n_spatial_groups))
"""
function scale_factor_vec_to_array(
    sf_vec::AbstractVector{T}, n_taxa::Int64, n_spatial_groups::Int64, n_p_types::Int64
)::Array{T,3} where T<:Union{Float64,String,Tuple}
    return reshape(sf_vec, (n_taxa, n_p_types, n_spatial_groups))
end

"""
    scale_factor_array_to_vec(sf_vec::Array{T, 3})::Vector{T} where T <: Union{Float64, String, Tuple}

Reshape a scale factor array of dimensions [taxa ⋅ param_type ⋅ spatial_groups] to a flatenned
...
"""
    generate_scale_factor_names(spatial_group_ids::Vector{Int64})::Array{String, 3}

Given a list of spatial_group generate the names of the columns corresponding to spatial_groups.

The format should be spatial_group_<id>_<param_name>_<taxa_name>
"""
function generate_scale_factor_names(spatial_group_ids::Vector{Int64})::Array{String,3}
    fg_names::Vector{Symbol} = ADRIA.functional_group_names()
    param_names::Tuple{Symbol,Symbol} = (:linear_extension, :mb_rate)

    n_spatial_groups::Int64 = length(spatial_group_ids)
    n_taxa::Int64 = length(fg_names)
    n_type_factors::Int64 = 2

    factor_names::Array{String,3} = Array{String,3}(
        undef, n_taxa, n_type_factors, n_spatial_groups
    )

    for (bg_idx, bg) in enumerate(spatial_group_ids),
        (pn_idx, pn) in enumerate(param_names),
        (fg_idx, fg) in enumerate(fg_names)

        factor_names[fg_idx, pn_idx, bg_idx] = "spatial_group_$(string(bg))_$(pn)_$(fg)"
    end
...
"""
    accel_params_vec_to_array(accel_params::AbstractVector{T}, n_spatial_groups::Int64)::Array{T, 2} where T <: Union{Float64, String, Tuple}
"""
function accel_params_vec_to_array(
    accel_params::AbstractVector{T},
    n_spatial_groups::Int64
)::Array{T,2} where T<:Union{Float64,String,Tuple}
    return reshape(accel_params, (n_spatial_groups, 3))
end

"""
    generate_growth_accel_names(spatial_group_ids::Vector{Int64})::Array{String, 2}

The format should be growth_accel_<param_name>_<spatial_group_id>. Return an array of dimensions
[spatial_groups ⋅ params]
"""
function generate_growth_accel_names(spatial_group_ids::Vector{Int64})::Array{String,2}
    param_names::Tuple{Symbol,Symbol,Symbol} = (:steepness, :height, :midpoint)

    n_params::Int64 = 3
    n_spatial_groups::Int64 = length(spatial_group_ids)

    factor_names::Matrix{String} = Matrix{String}(undef, n_spatial_groups, n_params)
    for (pn_idx, pn) in enumerate(param_names),
        (bg_idx, bg) in enumerate(spatial_group_ids)
...
function run_model(
    ...
    # Extract unique spatial_groups and creating location masks for each spatial_group
    unique_spatial_groups::Vector{Int64} = unique(domain.loc_data.GROUPED_BIOREGION)
    n_spatial_groups::Int64 = length(unique_spatial_groups)
    spatial_group_masks::BitMatrix = falses(n_locs, n_spatial_groups)
    for (idx, spatial_group) in enumerate(unique_spatial_groups)
        spatial_group_masks[:, idx] .= domain.loc_data.GROUPED_BIOREGION .== spatial_group
    end
    # Index into unique_spatial_groups for each location
    loc_spat_grp_idxs::Vector{Int64} = [
        findfirst(x -> x == spat_grp, unique_spatial_groups)
        for spat_grp in domain.loc_data.GROUPED_BIOREGION
    ]

    growth_acc_names::Vector{String} = accel_params_array_to_vec(
        generate_growth_accel_names(unique_spatial_groups)
    )
    growth_accel_parameters::Matrix{Float64} = accel_params_vec_to_array(
        param_set[factors=At(growth_acc_names)], n_spatial_groups
    )
    ...
    sf_col_names::Vector{String} = scale_factor_array_to_vec(
        generate_scale_factor_names(unique_spatial_groups)
    )
    # Extract scale factors array of dimensions [taxa ⋅ param_type · spatial_group_group]
    ...
    spat_grp_lin_ext::Array{Float64,3} = repeat(_linear_extensions, 1, 1, n_spatial_groups)
    spat_grp_survival::Array{Float64,3} = repeat(survival_rate, 1, 1, n_spatial_groups)
    for i in 1:n_spatial_groups
        spat_grp_lin_ext[:, :, i] .*= scale_factors[:, 1, i]
        spat_grp_survival[:, :, i] .= apply_mortality_scaling(
            spat_grp_survival[:, :, i], scale_factors[:, 2, i]
        )
    end
    ...
    for tstep::Int64 in 2:tf
        ...
        for idx in 1:n_spatial_groups
            growth_constraints[spatial_group_masks[:, idx]] .= linear_extension_scale_factors(
                C_cover_t[:, :, spatial_group_masks[:, idx]],
                vec_abs_k[spatial_group_masks[:, idx]],
                spat_grp_lin_ext[:, :, idx],
                _bin_edges,
                habitable_max_projected_cover[spatial_group_masks[:, idx]]
            )
        end
        ...
        for idx in 1:n_spatial_groups
            growth_constraints[no_constraint_mask .&& spatial_group_masks[:, idx]] .=
                growth_acceleration.(
                    growth_acc_height[idx],
                    growth_acc_midpoint[idx],
                    growth_acc_steepness[idx],
                    relative_habitable_cover[no_constraint_mask .&& spatial_group_masks[
                        :, idx
                    ]]
                )
        end
        ...
        for i in habitable_loc_idxs
            timestep!(
                functional_groups[i],
                recruitment[:, i],
                spat_grp_lin_ext[:, :, loc_spat_grp_idxs[i]] .* growth_constraints[i],
                spat_grp_survival[:, :, loc_spat_grp_idxs[i]]
            )
