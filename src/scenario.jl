...
function run_model(
    ...
    for tstep::Int64 in 2:tf
        ...
        if !isnothing(fog_strategy)
            state = if is_guided
                build_state(
                    domain, fog_strategy, loc_cover, recent_cover_losses,
                    last_fog_deployment
                )
            else
                nothing
            ...
            if !isempty(candidate_locs)
                ...
                if is_guided
                    ...
                    update_criteria_values!(
                        ...
                        coral_cover=loc_cover[candidate_loc_indices],
                ...
                if !isempty(selected_fog_ranks)
                    log_val = isguided ? (1:length(selected_fog_ranks)) : 1.0
        ...
        if !isnothing(seed_strategy)
            ...
            if !isempty(candidate_locs)
                ...
                if !isempty(selected_seed_ranks)
                    log_val = isguided ? (1:length(selected_seed_ranks)) : 1.0
        ...
        if has_seed_locs  # seed_decision_years[tstep] &&
            ...
            seed_locs = findall(domain.loc_ids .∈ [selected_seed_ranks])
            ...
            if length(locs_with_space) > 0
                ...
                proportional_increase, n_corals_seeded = distribute_seeded_corals(
                    vec_abs_k[seed_locs], available_space, seeded_area, seed_volume.data
                )
