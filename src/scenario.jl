...
Given a list of spatial group generate the names of the columns corresponding to spatial_groups.
...
function run_model(
    ...
    unique_spatial_groups::Vector{Int64} = unique(domain.loc_data.SPATIAL_GROUPING)
    ...
    for (idx, spatial_group) in enumerate(unique_spatial_groups)
        spatial_group_masks[:, idx] .= domain.loc_data.SPATIAL_GROUPING .== spatial_group
    end
    # Index into unique_spatial_groups for each location
    loc_sp_grp_idxs::Vector{Int64} = [
        findfirst(x -> x == sp_grp, unique_spatial_groups)
        for sp_grp in domain.loc_data.SPATIAL_GROUPING
    ]
    ...
    growth_accel_parameters::Matrix{Float64} = accel_params_vec_to_array(
        param_set[factors=At(growth_acc_names)], nspatial_groups_
    )
    ...
    # Extract scale factors array of dimensions [taxa ⋅ param_type · spatial_group]
    ...
    sp_grp_lin_ext::Array{Float64, 3} = repeat(_linear_extensions, 1, 1, n_spatial_groups)
    sp_grp_survival::Array{Float64, 3} = repeat(survival_rate, 1, 1, n_spatial_groups)
    for i in 1:nspatial_groups_
        sp_grp_lin_ext[:, :, i] .*= scale_factors[:, 1, i]
        sp_grp_survival[:, :, i] .= apply_mortality_scaling(
            sp_grp_survival[:, :, i], scale_factors[:, 2, i]
        )
    end
    ...
    for tstep::Int64 in 2:tf
        ...
        for idx in 1:nspatial_groups_
            growth_constraints[no_constraint_mask .&& spatial_group_masks[:, idx]] .= growth_acceleration.(
                growth_acc_height[idx],
                growth_acc_midpoint[idx],
                growth_acc_steepness[idx],
                relative_habitable_cover[no_constraint_mask .&& spatial_group_masks[:, idx]]
            )
        end
        ...
        for i in habitable_loc_idxs
            timestep!(
                functional_groups[i],
                recruitment[:, i],
                sp_grp_lin_ext[:, :, loc_sp_grp_idxs[i]] .* growth_constraints[i],
                sp_grp_survival[:, :, loc_sp_grp_idxs[i]]
            )
