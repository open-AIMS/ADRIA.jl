...
function rank_locations(
    ...
    for (scen_idx, scen) ∈ enumerate(eachrow(scens))
        ...
        if count(valid_fog_locs) > 0
            ...
            if !isempty(selected_fog_ranks)
                ranks_store[
                    ...
                    intervention=At(:fog),
                    scenarios=scen_idx
                ] .= 1:min(min_locs, length(selected_fog_ranks))
            end
        end
    end
...
function selection_score(
    ...
    lowest_ranks = maximum([maximum(r)
                            for r ∈ eachcol(ranks[intervention=At(iv_type)])])
...
function _calc_selection_score(
    ...
    if :timesteps in dims
        decisions = 0.0
        for s ∈ axes(tsliced, :scenarios)
            for t ∈ axes(tsliced, :timesteps)
...
function deployment_summary_stats(
    ...
    for (idx, scen_id) ∈ enumerate(iv_ranks.scenarios)
