...
function colony_areas()
    ...
    # Diameters in cm
    mean_cm_diameters =
        bin_edges[1:(end - 1)] + (bin_edges[2:end] - bin_edges[1:(end - 1)]) / 2.0
...
function coral_spec()::NamedTuple
    ...
    params.coral_id = String[join(x, "_") for x ∈ zip(tn, params.taxa_id, params.class_id)]
    ...
    params.growth_rate[params.class_id .== 6] .=
        params.growth_rate[params.class_id .== 6] .* 0.2
...
function create_coral_struct(bounds::Tuple{Float64,Float64}=(0.9, 1.1))::Nothing
    ...
    for c_id ∈ p_vals.coral_id
        for p ∈ base_coral_params
