...
## Loading ADRIA Results {#Loading-ADRIA-Results}

Results produced by `ADRIA.run_scenarios` are stored on disk and can be reloaded at any time using their path:
...
The returned `rs` is an `ADRIAResultSet` that gives access to everything needed for analysis and visualization.
...
### Accessing outcomes {#Accessing-outcomes}

Individual outcome arrays can be extracted directly from `rs.outcomes` or via the `ADRIA.metrics.*` functions (preferred):
...
See [Running scenarios](/usage/scenario_runs#Running-scenarios) and the [Metrics](/API#Metrics) page for more detail on available metrics and result set properties.

## Loading ReefModEngine Results {#Loading-ReefModEngine-Results}
...
To reduce duplication of geospatial and connectivity data, the data directory and results directory can be supplied separately to avoid keeping copies for each result set analysed.
