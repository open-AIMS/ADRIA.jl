...
This page covers functions provided by `ADRIAanalysis`. Ensure it is installed before running the examples (see [Getting Started](/usage/getting_started#Getting-Started)).
...
See the previous sections [Loading a Domain](/usage/loading_a_domain#Loading-a-Domain), [Generating scenarios](/usage/generating_scenarios#Generating-scenarios) and [Running scenarios](/usage/scenario_runs#Running-scenarios) for more information.

## Extracting results {#Extracting-results}

A range of metrics are defined as part of the ADRIA framework. See the [Metrics](/API#Metrics) page for more details.
...
Scenario-level metrics aggregate the above across the `site` dimension and indicate the _outcomes_ under a given intervention (or non-intervention) option and environmental condition. The result is a 2-dimensional array of timesteps and scenarios:
...
### Scenario outcomes {#Scenario-outcomes}
...
![](../assets/imgs/analysis/scenarios_tac.png)


### Intervention location selection - visualisation {#Intervention-location-selection-visualisation}
...
![](../assets/imgs/analysis/single_rank_plot.png)
...
![](../assets/imgs/analysis/ranks_plot.png)


## Intervention location selection - plot criteria maps {#Intervention-location-selection-plot-criteria-maps}
...
![](../assets/imgs/analysis/criteria_spatial_plots.png)


### PAWN sensitivity (heatmap overview) {#PAWN-sensitivity-heatmap-overview}
...
![](../assets/imgs/analysis/pawn_si.png)


### Temporal Sensitivity Analysis {#Temporal-Sensitivity-Analysis}
...
![](../assets/imgs/analysis/convergence_factors_series.png)
 
![](../assets/imgs/analysis/convergence_components_heatmap.png)
...
![](../assets/imgs/analysis/tsc_asv.png)
...
### Multiple Time Series Clustering {#Multiple-Time-Series-Clustering}
...
### Time Series Clustering Map {#Time-Series-Clustering-Map}
...
![](../assets/imgs/analysis/tsc_map.png)


### Rule Induction (using Series Clusters) {#Rule-Induction-using-Series-Clusters}

The SIRUS Rule Induction algorithm ([Benard et al. 2021](https://doi.org//10.1214/20-EJS1792)) can be used for scenario discovery by summarising scenarios in terms of binary rules, i.e. thresholds below/above which a factor will lead to a specified outcome.
...
### Regional Sensitivity Analysis {#Regional-Sensitivity-Analysis}
...
![](../assets/imgs/analysis/outcome_map.png)
...
The returned `DataFrame` has columns `feature`, `statistic`, `prob_superiority`, and `effect_size`, sorted descending by `prob_superiority`.
...
- **Inputs (`X`, the &quot;cost&quot;)** are the resources each scenario consumes and that you want to _minimise_ - for example deployment cost, effort, the number of corals deployed, or the area treated. ADRIA does not prescribe a cost metric: you supply a function (or a precomputed array) that returns one input value per scenario. Pass a `Vector{Float64}` of length `n_scenarios` for a single input, or a `Matrix{Float64}` of size `n_scenarios x n_inputs` for several. The `cost_function` in the example below is a placeholder for your own function.
  
- **Outputs (`Y`)** are the benefits each scenario produces and that you want to _maximise_ - metrics such as total coral cover and shelter volume.
...
![](../assets/imgs/analysis/example_dea_fig.png)


### GUI for high-level exploration (prototype only!) {#GUI-for-high-level-exploration-prototype-only!}
...
![](../assets/imgs/analysis/aviz_app.png)
