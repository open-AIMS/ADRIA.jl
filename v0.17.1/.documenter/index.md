...
## What is ADRIA.jl? {#What-is-ADRIA.jl?}
...
Common use cases include running scenarios under different environmental and intervention conditions (see [Loading a Domain](/usage/loading_a_domain#Loading-a-Domain), [Generating scenarios](/usage/generating_scenarios#Generating-scenarios) and [Running scenarios](/usage/scenario_runs#Running-scenarios)), extracting metrics for each scenario and timestep such as coral cover, shelter volume or number of juveniles (see [Metrics](/API#Metrics)), performing analyses like time series clustering (see [Analysis](/usage/analysis#Analysis)), and using the visualization tools to plot metrics and analysis results.

## Docker documentation {#Docker-documentation}

`ADRIA.jl` features a set of Docker build targets which allow usage, development and experimentation of the `ADRIA` Julia package. See [ADRIA Docker Setup](/development/docker#ADRIA-Docker-Setup) for more information.

## Troubleshooting F.A.Q {#Troubleshooting-F.A.Q}
...
- See `Extracting model details` example in [Cookbook examples](/usage/cookbook#Cookbook-examples) on how to extract the model specification and parameter table for a given domain.
  
- See also the `Generating and running scenarios` example in [Cookbook examples](/usage/cookbook#Cookbook-examples) which showcases how to run such a file for a given study area.
