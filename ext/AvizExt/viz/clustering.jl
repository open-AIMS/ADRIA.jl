...
function _cluster_legend_params(
    ...
    colors = [group_colors[key] for key ∈ group_keys]
    legend_entries = [PolyElement(; color=c, strokecolor=:transparent) for c ∈ colors]

    label_means::Vector{Float64} = zeros(length(group_keys))
    for (idx_key, key) ∈ enumerate(group_keys)
