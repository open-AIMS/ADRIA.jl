...
function ADRIA.viz.explore(rs::ResultSet)
    ...
    t_toggles = [
        ...
        active ∈ [has_45, has_60, has_85, has_cf, has_ug, has_g]
    ]
    t_toggle_map = zip(
        t_toggles,
    ...
    for (i, v) ∈ enumerate(eachrow(intervention_components))
        fn = v.name
    ...
    function update_disp(
        ...
        for (intv, bnds) ∈ enumerate(interv_labels)
    ...
    onany(time_slider.interval, tac_slider.interval,
        [t.active for t ∈ t_toggles]...,
        [sld.interval for sld ∈ interv_sliders]...
